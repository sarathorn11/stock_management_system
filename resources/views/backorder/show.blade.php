...
<div class="w-full h-full">
  <h1 class="text-2xl font-bold text-gray-800">Back Order Details - {{ $backOrder->bo_code }}</h1>
  <div class="flex items-center justify-between my-4">
    <div></div>
    <div class="flex items-center justify-between">
      <a class="inline-block bg-gray-300 text-black px-4 py-2 rounded mb-4 hover:bg-gray-400 hover:cursor-pointer ml-2">
        <i class="fa fa-print mr-2"></i>Print
      </a>
    </div>
  </div>
  <div class="card bg-white shadow-md p-6 rounded-sm">
    <div class="card-body" id="print_out">
        <div class="container-fluid">
            <div class="grid grid-flow-row gap-4 grid-cols-3">
              <div class="col-span-2">
                  <label class="text-blue-500">From P.O. Code</label>
                  <div>{{ $backOrder->po_code }}</div>
              </div>
    ...
    $(function(){
        $('#print').click(function(){
            ...
            var _head = $('head').clone()
                _head.find('title').text("Back Order Details - Print View")
            var p = $('#print_out').clone()
            p.find('tr.text-light').removeClass("text-light bg-navy")
            _el.append(_head)
            _el.append('<div class="d-flex justify-content-center">'+
                      '<div class="col-1 text-right">'+
                      '<img src="{{ asset('images/logo.png') }}" width="65px" height="65px" class="img-fluid mx-auto border-2 border-gray-400 rounded" />'+
                      '</div>'+
                      '<div class="col-10">'+
