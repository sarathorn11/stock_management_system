...
<div id="content" class="w-full h-full">
  <h1 class="text-2xl font-bold text-gray-800">List of Back Orders</h1>
  <div class="flex items-center justify-between my-4">
    <input id="search" type="text" placeholder="Search by BO Code..." class="px-3 py-2 w-[350px] rounded border">
      ...
      <tbody id="backOrderResults">
        @foreach($backOrders as $index => $row)
          <tr class="bg-white hover:bg-gray-200">
            ...
            <td class="p-4 text-center">{{ date('Y-m-d H:i', strtotime($row->date_created)) }}</td>
            <td class="p-4 text-center">{{ $row->bo_code }}</td>
            <td class="p-4 text-center">{{ $row->supplier }}</td>
            <td class="p-4 text-center">{{ $row->items->count() }}</td>
            <td class="p-4 text-center">
              @if($row->status == 0)
            ...
            <td class="p-4 flex items-center justify-center">
              <a href="{{ route('back-order.show', $row->id) }}" class="text-blue-500 text-[24px] mx-1">
                <i class="fa fa-eye mr-2"></i>
              </a>
              @if($row->status == 0)
                <a href="{{ route('back-order.edit', $row->id) }}" class="text-yellow-500 text-[24px] mx-1">
