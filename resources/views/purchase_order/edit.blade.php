...
@section('content')
<div>
    <h1 class="text-xl font-bold text-gray-800">Edit Purchase Order - {{$purchaseOrder->po_code}}</h1>
    <form id="receive-form" action="{{ route('purchase-order.update', $purchaseOrder->id) }}" method="POST" class="mt-8">
        @csrf
        @method('PUT')
        ...
        <table class="w-full border-collapse border border-gray-300">
            ...
            <tbody id="item-list">
                ...
                <tr>
                    ...
                    <td class="py-2 px-3 text-center border border-gray-400">
                        <input type="number" name="qty[]" class="w-12 border rounded-md p-1 text-center" value="{{ $item->quantity }}" min="0" onchange="updateRowTotal(this)">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        supplierSelect.value = "{{ $purchaseOrder->supplier_id }}"; // Set the selected supplier
        hiddenSupplierInput.value = supplierSelect.value; // Set the hidden input value
        fetchItemsBySupplier(supplierSelect.value); // Fetch items for the selected supplier
        // Disable the supplier select if there are items in the list
