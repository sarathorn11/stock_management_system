...
@section('content')
<div>
    <h1 class="text-xl font-bold text-gray-800">Create New Sale Record</h1>
    <form id="sale-form" action="{{ route('sales.store') }}" method="POST" class="mt-8">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="text-blue-500 font-medium">Sale Code</label>
                <input type="text" name="return_code" class="w-full border rounded-md p-2" value="{{ old('sales_code') }}" readonly>
            </div>
            <div>
                <label class="text-blue-500 font-medium">Client Name</label>
                <input type="text" name="client" class="w-full border rounded-md p-2" value="">
            </div>
        </div>
